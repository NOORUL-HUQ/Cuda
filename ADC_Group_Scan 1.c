...
IfxVadc_ChannelId g_vadcChannelIDs[] = {IfxVadc_ChannelId_0}; /* AN39: channel 4 of group 7                         */
...
//    {
        ...
        do
        {
            conversionResult = IfxVadc_Adc_getResult(&g_adcChannel[0]);
        } while(!conversionResult.B.VF); /* B for Bitfield access, VF for Valid Flag */

        /* Print the conversion to the UART */
        send_vadc(0, conversionResult.B.RESULT);
//    }
}
...
void send_vadc(uint32 chnIx, uint32 adcVal)
{
    ...
    char str1[TX_LENGTH] = {'s','h','.','X',':',' ','X','X','X','X','\n','\r'};
    char str2[TX_LENGTH] = {'k','h','.','X',':',' ','X','X','X','X','\n','\r'};/* X to be replaced by correct values*/
    ...
    /* Print via UART */
    //1send_data(str, TX_LENGTH);
    if(IfxPort_getPinState(BUTTON) == 0){
        flag++;
    }
    if(flag == 1 || flag == 3 || flag == 5 || flag == 7 ){
     if(adcVal> 2000){
         send_data(str1, TX_LENGTH);
     }
     else
     {
         send_data(str2, TX_LENGTH);
     }
